...
public class Recipe_sabanomisoni extends AppCompatActivity {
    ...
    protected void onCreate (Bundle savedInstanceState) {
        ...
        for (int i = 1; i < 11; i++) {
            reference.child("レシピ").child("鯖の味噌煮").child("材料" + i).addValueEventListener(listener);
        }

        DatabaseReference reference1 = FirebaseDatabase.getInstance().getReference();

        reference.child("レシピ").child("鯖の味噌煮").child("name").addValueEventListener(listener);

        DatabaseReference reference2 = FirebaseDatabase.getInstance().getReference();
        for (int i = 1; i < 10; i++) {
            reference.child("レシピ").child("鯖の味噌煮").child("作り方" + i).addValueEventListener(listener);

            DatabaseReference menu = FirebaseDatabase.getInstance().getReference();
            Button cooked = findViewById(R.id.cooked); //ボタンID
            menu.child("レシピ").child("鯖の味噌煮").child("used").addValueEventListener(menulistener);
            cooked.setOnClickListener(new View.OnClickListener(){
                public void onClick(View v)
                {
                    count += 1;
                    menu.child("レシピ").child("鯖の味噌煮").child("used").setValue(count);

                    Intent intent = new Intent(Recipe_sabanomisoni.this,Popup_questionnaire.class);
